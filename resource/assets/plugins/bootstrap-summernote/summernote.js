...
(function (factory) {
    ...
    var dom = (function () {
        ...
        var isControlSizing = function (node) {
            // modified by CJH.  2015/4/20
            return node && false; // $(node).hasClass('note-control-sizing');
            // --------------------
        };
...
// ------------------------------
        ...
        /**
         * added by C.J.H.   2015/3/20
         * 
         * insert glossary node.
         */
        this.createGlossary = function ($editable, linkInfo, options) {
            var linkGlossary = linkInfo.glossary;
            var linkText = linkInfo.text;
            var rng = linkInfo.range;
...
//            }
         ...
         * added by C.J.H.    2015/3/20
         * 
         * Get glossary info from Glossary node.
         */
        this.getGlossaryInfo = function($editable) {
            $editable.focus();
            ...
            return {
                range: rng,
                text: rng.toString(),
                glossary: $anchor.length ? $anchor.attr('glossary') : ''
            };
        };
    ...
    var Popover = function () {
        ...
        this.update = function ($popover, styleInfo, isAirMode) {
            ...
            var $glossaryPopover = $popover.find('.note-glossary-popover');
            if (styleInfo.anchor) {
                var glossary = $(styleInfo.anchor).attr('glossary');
                if ($(styleInfo.anchor).hasClass('glossary') && glossary!="") {
                    var $anchor = $glossaryPopover.find('a');
                    var $content = $glossaryPopover.find('div.note-glossary-content');
                    var text = $(styleInfo.anchor).html();
                    
                    // added by C.J.H.     2015/3/20
                    // show content when mouse on glossary.
                    $anchor.attr('href', '#').attr('glossary', glossary).html(text);
                    // ---------------------------------------
                    
                    $.ajax({
                        type: "POST",
                        url: $("#basePath").val() + 'glossary/detail.html',
                        data: { glossary_id: glossary},
                        dataType: 'json',
                        success: function (data) {
                            if (data.errCode == 0) {
                                
                                // added by C.J.H.   2015/7/2.  show result from database.
                                $anchor.attr('href', '#').attr('glossary', glossary).html(data.glossary.title);
                                $content.html(data.glossary.content);
                                // --------------------------------
                                
                                showPopover($glossaryPopover, posFromPlaceholder(styleInfo.anchor, isAirMode));
                            } else {
                            }
                        },
                        error: function () {
...
// ------------------------------
    ...
    // modified by CJH.  2015/04/20
    var GlossaryDialog = function(handler) {
        /**
         * toggle button status
         *
         * @private
        ...
        this.show = function (layoutInfo) {
            ...
            $.showGlossaryDialog($editable, $dialog, linkInfo).then(function (linkInfo) {
                handler.invoke('editor.restoreRange', $editable);
                handler.invoke('editor.createGlossary', $editable, linkInfo, options);
//                    // hide popover after creating link
                handler.invoke('popover.hide', $popover);
            }).fail(function () {
                handler.invoke('editor.restoreRange', $editable);
    ...
    var EventHandler = function () {
        ...
        var modules = this.modules = {
            ...
            glossaryDialog: new GlossaryDialog(this),
        ...
        var commands = {
             ...
             * Glossary Event.
             */
            showGlossaryDialog: function (layoutInfo) {
                modules.glossaryDialog.show(layoutInfo);
            },
...
//            }            
    ...
    var Renderer = function () {
        ...
        var tplButtonInfo = {
            ...
            glossary: function (lang) {
                return tplIconButton('fa fa-comments-o', {
                    event: 'showGlossaryDialog',
                    title: 'Glossary',
                    hide: true
                });
            }
        ...
        var tplPopovers = function (lang, options) {
            // add by C.J.H     2015/3/20
            // Glossary Popover Template
            var tplGlossaryPopover = function () {
                ...
                var content = '<a href="#" class="glossary">&nbsp;</a>&nbsp;&nbsp;' +
                        '<div class="note-insert btn-group">' +
                        linkButton + unlinkButton +
                        '</div>' +
                        '<div class="note-glossary-content"></div>'
                ;
                return tplPopover('note-glossary-popover', content);
            };
            // -----------------------------------------
            ...
            $notePopover.append(tplGlossaryPopover());
